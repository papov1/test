<?php
	...
	function updateRepositoryRelations($repository_id,$related_repositories,$lang_id){
		...
		if($related_repositories != ''){
			...
			foreach($related_repositories_array as $related_repository_name){
				$related_repositories_id[] = getRepositoryIdFromName(trim($related_repository_name),$lang_id);
			}
	...
	function updateUserEmail($user_id,$user_email,$user_email_old,$google_login_status){
		...
		$sql = "SELECT * FROM admin_allowed WHERE email = '".mysqli_real_escape_string($con,$user_email)."'";
		$result = mysqli_query($con,$sql);
		if(mysqli_num_rows($result) > 0){
			...
			$sql = "INSERT INTO admin_allowed SET email = '".mysqli_real_escape_string($con,$user_email)."', google_login_allowed = '".mysqli_real_escape_string($con,$google_login_status)."'";
			$result = mysqli_query($con,$sql);
		}
