	...
	function getSearchResult($id_lang,$keyword){
		...
		$sql = "
			...
			FROM
				repository
			JOIN
				repository_material_number
			ON
				repository.id = repository_material_number.repository_id
			WHERE
				repository.id_lang = $id_lang
			AND
				repository_material_number.material_number LIKE '%".mysqli_real_escape_string($con,$keyword)."%'
		";
		$result = mysqli_query($con,$sql);
