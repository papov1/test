$(document).ready(function () {
	...
	$('body').on('click','.lang_remove_validation_button',function(e){
		...
		if(verification_text == 'remove'){
			$.ajax({
				type: 'POST',
				url: "/admin/includes/pages/translations/ajax.php",
				data: {
			...
			window.fileUploader = new Dropzone("div#pdf_documents", { 
				...
				init: function() {
					...
					this.on("success", function() {
						...
						$('#new_repository_links_new').val('https://datenblatt.online/documents/'+new_file_name);
	...
	$('body').on('click','.goto_step_4',function(e){
		...
		$.ajax({
			type: 'POST',
			url: "/admin/includes/pages/repository/ajax.php",
	...
	$('body').on('click','.goto_step_9',function(e){
		...
		if($('#temp_file_url').val() != ''){
			var new_file_name = 'https://datenblatt.online/documents/'+$('#temp_file_url').val();
		}
		else{
			var new_file_name = '';
		}
			...
			onOpen: function() { 
				...
				window.fileUploader = new Dropzone("div#edit_pdf_documents", {
					...
					init: function() {
						...
						this.on("success", function() {
							$('#temp_file_url').val(new_file_name);
							$('#edit_repository_links_new').val('https://datenblatt.online/documents/'+new_file_name);
							//console.log('file uploaded: '+new_file_name);
							...
							$.ajax({
								...
								url: "/admin/includes/pages/repository/ajax.php",
								data: {
									file_uploaded: 'https://datenblatt.online/documents/'+new_file_name,
	...
	$('body').on('click','.save_login_with_google_email_address',function(e){
		...
		$.ajax({
			type: 'POST',
			url: "/admin/includes/pages/account/ajax.php",
			data: {
