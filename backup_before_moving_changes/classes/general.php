<?php
	...
	function getSearchResult($id_lang,$keyword){
		...
		//by name
        $sql = "SELECT `repository`.`name` 
                FROM `repository` 
                LEFT JOIN `repository_material_number` ON `repository`.`id` = `repository_material_number`.`repository_id` 
                LEFT JOIN `repository_tags` ON `repository`.`id` = `repository_tags`.`repository_id` 
                WHERE `repository`.`id_lang` = ".$id_lang." 
                AND (`repository`.`name` LIKE '%".mysqli_real_escape_string($con,$keyword)."%' OR `repository_material_number`.`material_number` LIKE '%".mysqli_real_escape_string($con,$keyword)."%' OR `repository_tags`.`name` REGEXP '[[:<:]]".mysqli_real_escape_string($con,$keyword)."[[:>:]]') 
